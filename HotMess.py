...
to 20m altitude, then navigate a heart shape. At each corner of the heart the drone
...
# This is used instead of 0 since distanceToWaypoint function is not 100% accurate
WAYPOINT_LIMIT = 1
...
def condition_yaw(heading, relative=False):
    ...
    After setting the yaw using this function there is no way to return to the default yaw "follow direction 
...
# Add your list of coordinates here
coordinates_list = [
    {'lat': 37.4219999, 'lon': -122.0840575},
    {'lat': 37.4222462, 'lon': -122.0841447},
    {'lat': 37.4225182, 'lon': -122.0841971},
]
...
for coord in coordinates_list:
    location = LocationGlobalRelative(coord['lat'], coord['lon'], TARGET_ALTITUDE)
    vehicle.simple_goto(location)
    while (distanceToWaypoint(location) > WAYPOINT_LIMIT):
        time.sleep(1)
    time.sleep(5)
